...
const channelcategory = [
  { label: 'Voice', key: 'Voice', icon: <UserOutlined /> },
  { label: 'SMS', key: 'sms', icon: <UserOutlined /> },
  { label: 'DATA', key: 'Data', icon: <UserOutlined /> },
...
// ---------------------------------------for category
  ...
  if (Array.isArray(validities)) {
    const cleanedValidities = validities.filter(
      (val) => val && val.valId && val.validity
    );
    setBundleList(cleanedValidities);
  }
}, [validities]);
...
const handleSubmit = async () => {
  ...
  if (result.success) {
    ...
    setBundleList(result.validities);
    ...
    return (
      ...
      >
        ...
        <td className="border px-4 py-2 text-center">
          <Button type="primary" size="small">{item.validity}</Button>
        </td>
        <td className="border px-4 py-2 text-center">
          <Button danger size="small" onClick={() => onDelete(index)}>Delete</Button>
        </td>
      </tr>
    );
  };
    ...
    <DndProvider backend={HTML5Backend}>
      <div className="absolute overflow-hidden top-14 px-5 inset-0 left-45 border border-gray-200 rounded-xl m-2 bg-gray-100 ">
        <div className="flex justify-between items-center py-2">
          <h1 className="py-2  text-2xl text-gray-500">Validity</h1>
            ...
            <table className="min-w-full border border-black text-sm">
              <thead>
                <tr className="bg-gray-200">
                  <th className="border px-4 py-2 text-center">Val ID</th>
                  <th className="border px-4 py-2 text-center">Validity</th>
              ...
              <tbody>
                {bundleList.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan={3} className="text-center py-4">No validities available</td>
                  </tr>
                )}
              </tbody>
            </table>
